...
(function () {
  ...
  /* ================================================================
     ...
     into every [data-version-slot] element in the panel. Removes the
     hardcoded-string drift that put stale versions in the header,
     About section, and footer even after the extension was patched.
  ================================================================ */
  try {
    var _mv = (chrome && chrome.runtime && chrome.runtime.getManifest &&
  ...
  */
  ...
  $('export-json').addEventListener('click', () => {
    downloadFile(JSON.stringify({
      tool: 'AMASAMYA', version: '5.2.0', page: auditMeta.pageTitle,
      /* v5.2.0: bumped from 4.3.1. Version alignment with platform. */
  ...
  /* v5.3.2 Accessible PDF: build the same semantic HTML we ship for
     the HTML export, inject an auto-print script, open in a new tab
     via a blob URL. The user picks Save as PDF in the print dialog;
     Chrome/Edge emit a tagged PDF preserving heading structure and
     reading order. Works in both Chrome MV3 side panel and Firefox
     sidebar. No third-party PDF library. */
  if ($('export-pdf')) {
    $('export-pdf').addEventListener('click', () => {
      ...
      if (!opened && chrome.tabs && chrome.tabs.create) {
...
<footer>Generated by AMASAMYA v5.2.0 - AMASAMYA.akhileshmalani.com - Akhilesh Malani</footer>
</body></html>`;
  }
